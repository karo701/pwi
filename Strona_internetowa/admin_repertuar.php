<?php
session_start();
require_once 'database.php';
if(isset($_POST['nazwa']))
{
    $wszystko_OK=true;
    $tytul = $_POST['nazwa'];
    if(empty($tytul))
    {
      $wszystko_OK=false;
      $_SESSION['e_tytul'] = "Podaj tytuł";
    }

    $sala = $_POST['sala'];
    if(empty($sala))
    {
      $wszystko_OK=false;
      $_SESSION['e_sala'] = "Podaj w jakiej sali";
    }

    $data = $_POST['data'];
    if(empty($data))
    {
      $wszystko_OK=false;
      $_SESSION['e_data'] = "Podaj datę";
    }

    $godzina = $_POST['czas'];
    if(empty($godzina))
    {
      $wszystko_OK=false;
      $_SESSION['e_godzina'] = "Podaj godzina";
    }

    $rodzaj = $_POST['rodzaj'];
    if(empty($rodzaj))
    {
      $wszystko_OK=false;
      $_SESSION['e_rodzaj'] = "Podaj rodzaj";
    }

    if($wszystko_OK==true)
    {
      try
      {
        $sql = "INSERT INTO repertuar (Tytul, Sala, Data, Godzina, Rodzaj_sztuki) VALUES ('$tytul', '$sala','$data','$godzina','$rodzaj')";
        $db->exec($sql);
        $_SESSION['udane'] ='Rekord został dodany';
      }
  catch(PDOException $error){
    $_SESSION['rror'] = $error;
    exist('id+adresu');
  }
}
}
?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <title>Opera i filharmonia</title>
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css" integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T" crossorigin="anonymous">
    <link href="style.css" type="text/css" rel="stylesheet">
</head>
<body>
        <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js" integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo" crossorigin="anonymous"></script>
        <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js" integrity="sha384-UO2eT0CpHqdSJQ6hJty5KVphtPhzWj9WO1clHTMGa3JDZwrnQq4sF86dIHNDz0W1" crossorigin="anonymous"></script>
        <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js" integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM" crossorigin="anonymous"></script>
        <header> </header>
        <nav class="navbar justify-content-center navbar-expand-lg navbar-dark bg-dark">
                   <a class="navbar-brand" href="index.php">HOME</a>
                   <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
                     <span class="navbar-toggler-icon"></span>
                   </button>
                   <div class="collapse navbar-collapse" id="navbarNav">
                     <ul class="navbar-nav">
                       <li class="nav-item active">
                         <a class="nav-link" href="repertuar.php">REPERTUAR</a>
                       </li>
                       <li class="nav-item">
                         <?php
                         if(isset($_SESSION['admin'])&&($_SESSION['admin']==true))
                          {
                            echo '<a class="nav-link" href="list.php">WIADOMOSCI OD USERA</a>';
                          }
                          else
                          {
                            echo '<a class="nav-link" href="kontakt.php">KONTAKT</a>';
                          }
                         ?>
                       </li>
                       <li class="nav-item">
                         <?php
                         if(isset($_SESSION['admin'])&&($_SESSION['admin']==true))
                          {
                            echo '<a class="nav-link" href="admin_repertuar.php">DODAJ REPERTUAR</a>';
                          }
                          else
                          {
                            if(!isset($_SESSION['zalogowany']))
                             {
                               echo '<a class="nav-link" href="rejestracja.php">REJESTRACJA</a>';
                             }
                          }
                         ?>
                       </li>
                       <li class="nav-item">
                         <?php
                         if(isset($_SESSION['zalogowany'])&&($_SESSION['zalogowany']==true))
                          {
                            echo '<a class="nav-link" href="logout.php">WYLOGUJ</a>';
                          }
                          else
                          {
                            echo '<a class="nav-link" href="logowanie.php">LOGOWANIE</a>';
                          }
                         ?>
                       </li>
                     </ul>
                   </div>
           </nav>
            <article>
                <section>
                  <br><br>
                  <h1>Panel dodawania repertuaru</h1>
                <!--   echo $_SESSION['tak'];-->
                    <h2><?php
                    if( isset($_SESSION['udane']))
                    {
                      echo $_SESSION['udane'];
                    }
                    else
                      echo "Uzupełnij";
                    ?></h2>
                  <form name="repertuar_a" method="post">
                    <label>Tytuł sztuki</label>
                    <input type="text" name="nazwa" />
                    <?php
                       if(isset($_SESSION['e_tytul']))
                       {
                         echo '<div>'.$_SESSION['e_tytul'].'</div>';
                          unset($_SESSION['e_tytul']);
                       }
                    ?>
                    <label>Sala</label>
                    <input type="text" name="sala"/>
                    <?php
                       if(isset($_SESSION['e_sala']))
                       {
                         echo '<div>'.$_SESSION['e_sala'].'</div>';
                          unset($_SESSION['e_sala']);
                       }
                    ?>
                		<label>Data</label>
                		<input type="date" name="data"/>
                    <?php
                       if(isset($_SESSION['e_data']))
                       {
                         echo '<div>'.$_SESSION['e_data'].'</div>';
                          unset($_SESSION['e_data']);
                       }
                    ?>
                    <label>Godzina</label>
                    <input type="time" name="czas"/>
                    <?php
                       if(isset($_SESSION['e_godzina']))
                       {
                         echo '<div>'.$_SESSION['e_godzina'].'</div>';
                          unset($_SESSION['e_godzina']);
                       }
                    ?>
                    <label>Rodzaj sztuki</label>
                    <input type="text" name="rodzaj"/>
                    <?php
                       if(isset($_SESSION['e_rodzaj']))
                       {
                         echo '<div>'.$_SESSION['e_rodzaj'].'</div>';
                          unset($_SESSION['e_rodzaj']);
                       }
                    ?>
                    <input type="submit" class="form-cont submit" value="Dodaj">
                    <?php
                      unset($_SESSION['udane']);
                    ?>
                  </form>
                </section>
            </article>
<aside></aside>

<footer>
    <h3>Karolina Szymańska</h3>
</footer>


</body>
</html>
